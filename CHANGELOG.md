...
- 扩展 6 大预热展区资产（RAG、大模型微调、视觉大模型、AI Agent、推理加速量化、强化学习思维链），平均包含 50–80 个节点与 50–80 条时序演化边，全面呈现茂盛生长的知识世界树形态。
- 新增植物隐喻视觉保护机制：点击节点高亮聚焦时，上下文中的花（问题）、果（观点）、叶（来源）保持完整植物具象形态，不再退化为单一圆形几何底板。
- 新增空白处点击及 ESC 键完整退出聚焦交互，一键复原全局知识世界树。
- 新增 Signal Garden 前端视觉方案：沉浸式知识演化画布、信号扫描层、状态微动效、画布操作提示和移动端重排。

### Changed

- 优化时间轴与播放控制逻辑：时间滑块由底部移至控制栏，修复点击暂停后再播放时时间重置问题，支持无缝断点续播。
- 移除展示区域冗余的左半部分时间刻度线，最大化知识场画布展示面积与交互视野。
- 仅在关键重点节点展示标签文字，非重点节点自动弱化标签，大幅提升大图谱浏览的清晰度与茂盛感。
- 将树节点聚焦从矩形光晕改为轮廓高亮，收敛为单一薄荷绿主强调色。
