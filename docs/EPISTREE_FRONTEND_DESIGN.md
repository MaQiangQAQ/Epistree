...
> 版本：v0.3 Signal Garden（2026-09-10）
> 状态：第二版视觉方案已落地，待用户体验反馈
...
### v0.3 视觉方向：Signal Garden

本轮保留 Dash 与 Dash Cytoscape，不改数据契约，将原有均匀分栏改成“知识演化控制台”：中央图谱是唯一主舞台，左侧是紧凑知识语法与时间信号轨道，右侧是节点情报和知乎来源抽屉，底部时间轴承担播放与筛选。视觉采用深黑背景、薄荷绿单主色、低对比边界和克制光效，避免多色渐变与大面积玻璃卡片造成廉价感。

参考方案及采用部分：

- [Vercel Geist](https://vercel.com/geist)：以间距、字体与细边界建立层级，色彩仅表达状态。
- [Linear](https://linear.app/)：暗色优先、键盘友好、紧凑而安静的工具界面。
- [OneZoom](https://www.onezoom.org/)：让树形结构占据视觉中心，界面控件退居辅助位置。
- [Gource](https://github.com/acaudwell/Gource)：时间驱动的生长叙事与深色空间氛围。
- [Cytoscape.js](https://js.cytoscape.org/)：继续使用声明式样式、预设布局和增量元素更新，不引入自研 Canvas。

动效限定为 180–750 ms 的入场、悬停、状态脉冲与时间播放，环境扫描线为低频循环；系统开启 `prefers-reduced-motion` 时自动关闭持续动画。移动端按“搜索操作—图谱—详情—图例”重排，不简单压缩桌面三栏。
